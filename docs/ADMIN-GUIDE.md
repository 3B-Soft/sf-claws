...
## Clients and orgs
Create a client, add its orgs (production is auto-protected), connect each org with "Connect to Salesforce". The extension resolves the org from the browser tab's MyDomain host, so orgs must be connected before admins can use them.
