...
Register models with their prices; bind each agent role to a model with an effort level and iteration cap. Thinking effort is a super-admin setting, and end users do not tune inference. The `orchestrator` binding's effort is the ceiling for the whole session: when the lead agent delegates (`run_subagent`, `consult_specialist`) it can pick a lower effort for that worker, such as `low` for a lookup or `high` for design. If it picks none, the worker's own role binding applies. Either way a worker never runs above the orchestrator's effort. Temperature and Top P are optional per-model dials — blank means the provider default, which is what most deployments want, and both are unavailable on a thinking model because the providers reject them alongside extended reasoning. Cheap models for `analyst`, `reviewer`, `doc_writer`, `summarizer`; strongest model for `orchestrator`, `metadata_builder`, `flow_builder`, `apex_builder`. Use "Test connection" after entering a key.
