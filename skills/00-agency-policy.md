...
- **Minimal Touch Principle:** Do not modify existing Apex classes, flows, or metadata unless directly required by the user's task.
    - **No cosmetic edits:** Never touch an existing file solely to add/update inline comments, reformat code, or fix minor formatting/linting issues.
    - **No arbitrary API version upgrades:** Do not bump the `apiVersion` in `.cls-meta.xml` or flow definitions unless the feature you are implementing requires a higher API version.
    - **Prefer extension over modification:** When adding new behavior, prefer adding new, isolated components or unmanaged extension fields over editing established legacy classes or flows.
