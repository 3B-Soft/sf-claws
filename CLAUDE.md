...
- Monorepo (Bun workspaces, Bun 1.3.12). Build order: shared → server → admin-ui → extension.
  `bun run lint && bun run typecheck && bun run test` is what CI runs; run all three before pushing.
- Contract first: change `packages/shared/src/*.ts` (zod) before touching server routes or UI code;
  rebuild shared (`bun run --filter @sf-claws/shared build`) so the UIs pick up `dist/`.
- Server: Fastify + bun:sqlite, ESM with `.js` import suffixes. Add schema changes as a **new**
...
- Colours are **semantic tokens only** (`bg-surface`, `text-content-muted`, `border-line`,
  ...
  styling change run `bun run contrast` — it fails any text under WCAG AA on its real background.
